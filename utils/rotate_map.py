...
def rotate_graph(data):
    ...
    # --- Rotate nodes ---
    new_nodes = {}
    for old_id_str, coords in data["nodes"].items():
        old_id = int(old_id_str)
        new_id = old_to_new[old_id]
        new_coords = rotate_node_coords(coords[0], coords[1])
        new_nodes[str(new_id)] = new_coords

    # Sort nodes by integer key for clean output
    sorted_nodes = dict(sorted(new_nodes.items(), key=lambda item: int(item[0])))

    # --- Rotate edges ---
    new_edges = []
    for edge in data["edges"]:
        new_u = old_to_new[edge["u"]]
        new_v = old_to_new[edge["v"]]

        # Remap attributes (adjacent node IDs), preserving None values
        new_attributes = []
        for attr in edge["attributes"]:
            if attr is None:
                new_attributes.append(None)
            else:
                new_attributes.append(old_to_new[attr])
        ...
        new_edge = {
            ...
            "edge_type": rotate_edge_type(edge["edge_type"]),
        }
        new_edges.append(new_edge)

    # Sort edges by (u, v) for consistent ordering
    new_edges.sort(key=lambda e: (e["u"], e["v"]))

    # --- Build output ---
    result = {
        "metadata": {
            "rows": cols,   # rotated: new rows = old cols
            "cols": rows,   # rotated: new cols = old rows
        },
        "nodes": sorted_nodes,
        "edges": new_edges,
    }
